...
void gradientFirst (vector<double> input) {
    ...
    while (flag) {
        ...
        if (max < epsVal) {
            printf("Point: %f, %f, %f\nvalue: %f", vectorNew[0], vectorNew[1], vectorNew[2], 2 * pow(vectorNew[0], 2.0) + 2 * pow(vectorNew[1], 2.0) + vectorNew[2]*vectorNew[2] - 2 * vectorNew[0] * vectorNew[1] - 2 * vectorNew[2] *vectorNew[1] - 2* vectorNew[0] + 3);
            flag = false;
        }
        input = vectorNew;
...
void gradientSecond (vector<double> input) {
    ...
    while (flag) {
        ...
        if (max < epsVal) {
            printf("Point: %f, %f, %f\nvalue: %f", vectorNew[0], vectorNew[1], vectorNew[0]*vectorNew[0]*vectorNew[0]*vectorNew[0] * 3 + 4* vectorNew[0]*vectorNew[0]*vectorNew[0] - 12 * vectorNew[0]*vectorNew[0] + 12*vectorNew[1] * vectorNew[1] - 24 * vectorNew[1]);
            flag = false;
        }
        input = vectorNew;
