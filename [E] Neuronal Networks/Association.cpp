...
vector<double> getClearVector_T3(double val){
    vector<double> tmp;
    for (int i = 0; i<25; i++) {
        tmp.push_back(val);
    }
...
vector<vector<double> > createWvector(vector<double> w, vector<double> u) {
    ...
    getClearVector_T3(-1.0);

    for (int i=0; i<25; i++) {
        output.push_back(getClearVector_T3(-1.0));
        for (int j=0; j<25; j++) {
...
void printVector_T3(vector<double> vec) {
    for (int i=0; i<25; i++) {
        if (vec[i] == -1.0) {
            printf("[] ");
        } else {
